...
{
    /**
     * Salary Grade Table – SSL VI / EO 64 (2026, Tranche 3)
     *
     * 1. Creates (or finds) the PayrollSslTable header record
     * 2. Upserts all 33 × 8 steps linked to that header
     * Safe to re-run.
     */
    public function run(): void
    {
        ...
        // ── 1. Create the SSL table header ────────────────────────────────────
        $sslTableId = DB::table('payroll_ssl_tables')
            ->where('ssl_version', 'SSL VI')
            ->where('tranche', 3)
            ->value('ssl_table_id');

        if (! $sslTableId) {
            $sslTableId = DB::table('payroll_ssl_tables')->insertGetId([
                'ssl_version' => 'SSL VI',
                'legal_basis' => 'RA 11466 / EO 64',
                'tranche' => 3,
        ...
        // ── 2. Salary data (SG 33 Steps 3–8 are null / not applicable) ───────
        $table = [
            ...
            // SG 33: only steps 1–2 have official values; steps 3–8 are null
            [33, [449157, 462329, null, null, null, null, null, null]],
        ];

        // ── 3. Upsert all 264 rows ────────────────────────────────────────────
        foreach ($table as [$sg, $salaries]) {
            // Pad to exactly 8 steps (handles SG33's short array)
            $salaries = array_pad($salaries, 8, null);

            foreach ($salaries as $stepIndex => $amount) {
                $step = $stepIndex + 1;

                $existing = DB::table('salary_grade_steps')
                    ->where('ssl_table_id', $sslTableId)
                    ->where('salary_grade', $sg)
                    ->where('step', $step)
                    ->value('salary_grade_step_id');

                if ($existing) {
                    DB::table('salary_grade_steps')
                        ->where('salary_grade_step_id', $existing)
                        ->update([
                            'monthly_salary' => $amount,
                            'updated_at' => $now,
                        ]);
                } else {
                    DB::table('salary_grade_steps')->insert([
                        'ssl_table_id' => $sslTableId,
                        'salary_grade' => $sg,
                        'step' => $step,
                        'monthly_salary' => $amount,
                        'created_at' => $now,
