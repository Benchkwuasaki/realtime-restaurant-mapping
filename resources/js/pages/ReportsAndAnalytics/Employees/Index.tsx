...
const Card = ({ children, style = {}, className = '' }: { children: React.ReactNode; style?: React.CSSProperties; className?: string }) => (
    <div className={className} style={{ background: 'var(--card)', borderRadius: 16, border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)', padding: 20, ...style }}>
        {children}
    </div>
);
...
function ActiveFilters({ filters, setFilters }: { filters: EmployeeFilters; setFilters: (f: EmployeeFilters) => void }) {
    ...
    return (
        <div className="flex flex-wrap items-center gap-2 rounded-xl border border-border bg-muted px-3 py-2.5">
...
/* ══════════════════════════════════════════
   2. DEMOGRAPHICS
   Mobile: 1-col stacked | md+: 3-col grid
══════════════════════════════════════════ */
    ...
    return (
        <Card>
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
                    ...
                    {ageBuckets.map((b, i) => (
                        <div key={b.label} className="mb-3">
                            <div className="mb-1 flex justify-between">
                                <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--foreground)' }}>{b.label}</span>
                                <span style={{ fontSize: 12, fontWeight: 700, color: AGE_COLORS[i] }}>{b.count}</span>
                            </div>
                            <div style={{ height: 16, background: 'var(--muted)', borderRadius: 6, overflow: 'hidden' }}>
                                <div style={{ width: `${(b.count / maxAge) * 100}%`, height: '100%', background: AGE_COLORS[i], borderRadius: 6, transition: 'width .4s' }} />
                            </div>
                        </div>
                    ))}
                </div>
                ...
                {/* Gender */}
                <div>
                    <div className="mb-3 text-[11px] font-bold uppercase tracking-wider text-muted-foreground">Gender</div>
                    {genderCounts.map(g => (
                        <div key={g.label} className="mb-4">
                            ...
                            <div style={{ height: 10, background: 'var(--muted)', borderRadius: 6, overflow: 'hidden' }}>
                                <div style={{ width: `${total ? (g.count / total) * 100 : 0}%`, height: '100%', background: GENDER_COLORS[g.label], borderRadius: 6, transition: 'width .4s' }} />
                            </div>
                        </div>
                    ))}
                </div>
                                ...
                                <Tooltip>
                                    <TooltipTrigger asChild>
                                        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, height: '100%', justifyContent: 'flex-end', cursor: 'default' }}>
                                            ...
                                            <div style={{ width: '100%', background: [emerald, cyan, blue, violet, amber, rose][i % 6], borderRadius: '4px 4px 0 0', height: `${(e.count / maxEduc) * 100}px`, transition: 'height .4s' }} />
                ...
                {counts.map(c => {
                    ...
                    return (
                        <div key={c.label}
                            ...
                            style={{
                                background: `${c.color}10`, border: `1px solid ${isActive ? c.color : `${c.color}30`}`,
            ...
            <TooltipProvider>
                <div style={{ display: 'flex', height: 12, borderRadius: 8, overflow: 'hidden', gap: 1 }}>
                    {counts.map(c => (
                        <Tooltip key={c.label}>
                            <TooltipTrigger asChild>
                                <div onClick={() => toggle(c.label)} style={{ width: `${total ? (c.count / total) * 100 : 0}%`, background: c.color, transition: 'width .4s, opacity .15s', cursor: 'pointer', opacity: filters.type && filters.type !== c.label ? 0.3 : 1 }} />
                            </TooltipTrigger>
                ...
                {counts.map(d => {
                    ...
                    return (
                        <TooltipProvider key={d.label}>
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    <div onClick={() => toggle(d.label)}
                                        onMouseEnter={() => setHov(d.label)}
                                        onMouseLeave={() => setHov(null)}
                                        style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', opacity: filters.dept && !isActive ? 0.4 : 1, transition: 'opacity .15s' }}>
                                        {/* Label — narrower on mobile */}
                                        <div style={{ width: 90, fontSize: 11, fontWeight: isActive ? 800 : 600, color: isActive ? d.color : 'var(--foreground)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', flexShrink: 0 }}>{d.label}</div>
                                        <div style={{ flex: 1, height: 20, background: 'var(--muted)', borderRadius: 6, overflow: 'hidden', minWidth: 0 }}>
                                            <div style={{ width: `${(d.count / max) * 100}%`, height: '100%', background: d.color, borderRadius: 6, display: 'flex', alignItems: 'center', paddingLeft: 7, transition: 'width .4s', opacity: hov && hov !== d.label && !isActive ? 0.6 : 1, outline: isActive ? `2px solid ${d.color}` : 'none', outlineOffset: 1 }}>
                                                <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--primary-foreground)', whiteSpace: 'nowrap' }}>{d.count}</span>
                                            </div>
                                        </div>
                                        <div style={{ width: 28, fontSize: 11, fontWeight: 700, color: d.color, textAlign: 'right', flexShrink: 0 }}>{d.count}</div>
                                    </div>
                                </TooltipTrigger>
                ...
                {STATUSES.map(s => {
                    ...
                    return (
                        <div key={s}
                            ...
                            <div style={{ fontSize: 18, color: cfg.color, marginBottom: 4 }}>{cfg.icon}</div>
    ...
    return (
        <Card>
            <div className="mb-4 flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                ...
                <Button variant="outline" size="sm" onClick={exportCSV} className="self-start sm:self-auto">⬇ Export CSV</Button>
            </div>
            <DataTable
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex flex-col gap-4 overflow-y-auto p-3 sm:p-6" style={{ fontFamily: 'var(--font-sans)' }}>
                ...
                {/* Type + Status — 1 col mobile, 2 col md+ */}
