...
// Required fields per step (steps 0 & 1 only — others are optional collections)
...
function EmptyState({ label }: { label: string }) {
    return (
        <p className="text-sm text-muted-foreground italic py-2">{label}</p>
    )
}

// ─── Reusable add/remove row wrapper ─────────────────────────────────────────
...
interface PersonalStepProps {
    data: {
        ...
        name_extension: string; birth_date: string; sex: string
        civil_status: string; place_of_birth: string
...
function PersonalStep({ data, setData, err }: PersonalStepProps) {
    return (
        <div className="grid grid-cols-3 gap-5">
            ...
            <div className="space-y-2">
                ...
                <FieldError message={err("middle_name")} />
            </div>
            <div className="space-y-2">
                ...
                <FieldError message={err("name_extension")} />
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
                ...
                <FieldError message={err("place_of_birth")} />
            </div>
            <div className="space-y-2">
...
interface EmploymentStepProps {
    data: {
        item_id: string; salary_grade_step_id: string
        employment_classification: string; work_email: string
        password: string; date_applied: string; date_hired: string
        work_schedule_start: string; work_schedule_end: string; status: string
        salary_grade: string; step: string
    }
    setData: SetDataFn
    err: ErrFn
    items: Item[]
...
function EmploymentStep({ data, setData, err, items, salaryGradeSteps }: EmploymentStepProps) {
    return (
        <div className="grid grid-cols-3 gap-5">
            ...
            <div className="space-y-2">
                ...
                <Select value={data.employment_classification} onValueChange={(v) => setData("employment_classification", v)}>
                    <SelectTrigger id="employment_classification"><SelectValue placeholder="Select classification" /></SelectTrigger>
                    <SelectContent>
            ...
            <div className="space-y-2">
                ...
                <Select value={data.salary_grade_step_id} onValueChange={(v) => setData("salary_grade_step_id", v)} disabled={!data.salary_grade}>
                    <SelectTrigger id="salary_grade_step_id"><SelectValue placeholder={data.salary_grade ? "Select step" : "Select a grade first"} /></SelectTrigger>
                    <SelectContent>
                        {salaryGradeSteps.filter((s) => String(s.salary_grade) === data.salary_grade).sort((a, b) => a.step - b.step).map((sgs) => (
...
function GovernmentStep({ rows, setRows }: { rows: GovernmentRow[]; setRows: (r: GovernmentRow[]) => void }) {
    ...
    return (
        <CollectionSection
            title="Add government account numbers (SSS, PhilHealth, GSIS, TIN, Pag-IBIG, etc.)."
            onAdd={add}
...
function ReviewStep({ data, items, salaryGradeSteps, addresses, family, government, education, eligibility }: ReviewStepProps) {
    ...
    return (
        <div className="space-y-0.5">
            ...
            {addresses.length === 0
                ...
                : addresses.map((a, i) => (
                    <div key={i} className="text-sm py-0.5 text-foreground">
                        {i + 1}. {[a.street_address, a.city, a.state, a.zip_code].filter(Boolean).join(", ")}
                    </div>
                ))}
            ...
            {family.length === 0
                ...
                : family.map((f, i) => (
                    <div key={i} className="text-sm py-0.5 text-foreground">
                        {i + 1}. {f.full_name} {f.relationship && `(${f.relationship})`} {f.contact_number && `— ${f.contact_number}`}
                    </div>
                ))}
            ...
            {education.length === 0
                ...
                : education.map((e, i) => (
                    <div key={i} className="text-sm py-0.5 text-foreground">
                        {i + 1}. {e.level} {e.degree && `— ${e.degree}`}, {e.school_name} {e.graduation_date && `(${e.graduation_date})`}
                    </div>
                ))}
            ...
            {eligibility.length === 0
                ...
                : eligibility.map((e, i) => (
                    <div key={i} className="text-sm py-0.5 text-foreground">
                        {i + 1}. {e.eligibility_name} {e.year_passed && `— Passed: ${e.year_passed}`}
                    </div>
                ))}
        </div>
    )
...
export default function CreateEmployee({ items, salaryGradeSteps }: Props) {
    ...
    // Collection state (outside useForm since Inertia useForm doesn't handle nested arrays well)
            ...
            <div className="px-10 pt-5">
                ...
                <form onSubmit={submit}>
                    <div className="mt-8 p-6 border rounded-md">
                        ...
                        {currentStep === 1 && <EmploymentStep data={data} setData={setData} err={err} items={items ?? []} salaryGradeSteps={salaryGradeSteps ?? []} />}
