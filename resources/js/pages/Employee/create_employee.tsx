...
function RowCard({ onRemove, children }: { onRemove: () => void; children: React.ReactNode }) {
    return (
        <div className="relative border rounded-md p-4 bg-muted/30">
            <button type="button" onClick={onRemove} className="absolute top-3 right-3 text-muted-foreground hover:text-destructive transition-colors" aria-label="Remove">
...
function PersonalStep({ data, setData, err }: {
    data: { first_name: string; last_name: string; middle_name: string; name_extension: string; birth_date: string; sex: string; civil_status: string; place_of_birth: string; personal_email: string; phone_number: string }
...
function ReviewStep({ data, items, salaryGradeSteps, addresses, family, government, education, eligibility }: {
    ...
    return (
        <div className="space-y-0.5">
            <p className="text-sm text-muted-foreground mb-1">Review all information below. Use <strong>Previous</strong> to go back and make changes.</p>
...
export default function CreateEmployeeForm({ items, salaryGradeSteps }: CreateEmployeeProps) {
